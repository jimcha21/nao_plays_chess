...
public:
    ...
    : it_(nh_)
  {
    ...
    //image_sub_ = it_.subscribecd("/naoqi_driver_node/camera/front/image_raw", 10, &ImageConverter::imageCb, this);
    image_sub_ = it_.subscribe("/usb_cam/image_raw", 10, &ImageConverter::imageCb, this);    
    //image_pub_ = it_.advertise("/image_converter/output_video", 1);

    //cv::namedWindow(cv::OPENCV_WINDOW);
  }

  ~ImageConverter()
  {
    //cv::destroyWindow(cv::OPENCV_WINDOW);
  }
...
int main(int argc, char* argv[])
{
    ...
    FileStorage fs("default.xml", FileStorage::READ); // Read the settings
    if (!fs.isOpened())
    {
